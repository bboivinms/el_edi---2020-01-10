...
namespace EDI_RSS
{
    public class Email810Writer : EDI_RSS.Program_Base
    {
        ...
        public Email810Writer(string Id)
        {
            ...
            Htmldoc = File.ReadAllText(@"template810Email.html");
            program810Id = Id;
        }
        ...
        private void SendInternalEmail()
        {
            ...
            MailAddress from = new MailAddress("web2@envl.ca", "EDI (Multi-services)", System.Text.Encoding.UTF8);
            MailAddress to = null;
            ...
            if (UseSystem == "live")
            {
                if (IDE_status == "send")
                {
                    to = new MailAddress("payable@multi-services.org"); 
                }
            }
            else
            {
