...
namespace vivael
{
    public class GanttChart : UserControl
    {
        ...
        public void AddChartBar(int rowHeight, string rowText, string innerText, object barValue, DateTime fromTime, DateTime toTime, Color color, Color hoverColor, int rowIndex)
        {
            ChartBarDate bar = new ChartBarDate();
            bar.RowHeight = rowHeight;
            bar.Text = rowText;
            bar.InnerText = innerText;
            bar.Value = barValue;
        ...
        public void AddChartBar(string rowText, string innerText, object barValue, DateTime fromTime, DateTime toTime, Color color, Color hoverColor, int rowIndex, bool hideFromMouseMove)
        {
            ChartBarDate bar = new ChartBarDate();
            bar.Text = rowText;
            bar.InnerText = innerText;
            ...
            foreach (ChartBarDate bar in bars)
            {
                ...
                if (bar.StartValue != null & endValue != null)
                {
                    ...
                    if ((index >= scrollPos & index < barsViewable + scrollPos) | ignoreScrollAndMousePosition == true)
                    {
                        ...
                        // grfx.DrawString(bar.InnerText, rowTextFont, Brushes.White, obRect.X + 5, 5 + barStartTop + (barHeight * (index - scrollPos)) + (barSpace * (index - scrollPos)));

                        grfx.DrawString(bar.Text, rowTextFont, Brushes.Black, 0, barStartTop + (currentHeight) + (barSpace * (index - scrollPos)));
                        grfx.DrawString(bar.InnerText, rowTextFont, Brushes.White, obRect.X + 5, 5 + barStartTop + (currentHeight) + (barSpace * (index - scrollPos)));
        ...
        public void DrawNetHorizontal(Graphics grfx)
        {
            ...
            for (index = 0; index <= GetIndexChartBar("QQQQQQ"); index++)
            {
                foreach (ChartBarDate bar in bars)
                {
                    ...
                    grfx.DrawLine(lineColor, 0, barStartTop + (currentHeight) + (barSpace * index), width, barStartTop + (currentHeight) + (barSpace * index));
                }
                
                currentHeight += bars[index].RowHeight;
            }

            // lastLineStop = barStartTop + (currentHeight- barHeight) + (barSpace * (index - 1));
            lastLineStop = barStartTop + (currentHeight) + (barSpace * (index - 1));
        }
